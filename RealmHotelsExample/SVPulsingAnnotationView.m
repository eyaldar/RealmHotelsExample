...
@property (nonatomic, strong) CAGradientLayer *outerDotLayer;
...
- (void)setHeadingImage:(UIImage *)image {
    ...
    self.headingImageView.image = image;
...
- (CALayer*)outerDotLayer {
    if(!_outerDotLayer) {
        _outerDotLayer = [CAGradientLayer layer];
        _outerDotLayer.bounds = self.bounds;
        _outerDotLayer.colors = [NSArray arrayWithObjects:(id)[UIColor yellowColor].CGColor,
                                                          (id)[UIColor orangeColor].CGColor,
                                                          (id)[UIColor yellowColor].CGColor,
                                                          nil];
        _outerDotLayer.startPoint = CGPointMake(0, 0);
        _outerDotLayer.endPoint = CGPointMake(0, 1);
        _outerDotLayer.cornerRadius = self.bounds.size.width/2;
...
- (CALayer*)colorDotLayer {
    if(!_colorDotLayer) {
        ...
        if (_image == nil) {
            ...
            [_colorDotLayer setFrame:CGRectMake(0, 0, width, width)];
            _colorDotLayer.contents = (id)_image.CGImage;
        }
        _colorDotLayer.cornerRadius = width/2;
        _colorDotLayer.masksToBounds = true;
